class MtnCiCashoutsController < ApplicationController
  layout :layout_used

  def save_log
    cashout_log = MtnCiCashout.new(params.except(:controller, :action))
    status = "0"
  ...
  def list
    @cashouts = MtnCiCashout.all.order("created_at DESC")
  end

  def validate_cashout
    cashout = MtnCiCashout.find_by_transaction_id(params[:transaction_id])
