class NovapayCashoutsController < ApplicationController
  layout :layout_used

  def save_log
    cashout_log = NovapayCashout.new(params.except(:controller, :action))
    status = "0"
  ...
  def list
    @cashouts = NovapayCashout.all.order("created_at DESC")
  end

  def validate_cashout
    cashout = NovapayCashout.find_by_transaction_id(params[:transaction_id])
