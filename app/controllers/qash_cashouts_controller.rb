class QashCashoutsController < ApplicationController
  layout :layout_used

  def save_log
    cashout_log = QashCashout.new(params.except(:controller, :action))
    status = "0"
  ...
  def list
    @cashouts = QashCashout.all.order("created_at DESC")
  end

  def validate_cashout
    cashout = QashCashout.find_by_transaction_id(params[:transaction_id])
