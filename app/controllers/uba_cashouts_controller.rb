class UbaCashoutsController < ApplicationController
  layout :layout_used

  def save_log
    cashout_log = UbaCashout.new(params.except(:controller, :action))
    status = "0"
  ...
  def list
    @cashouts = UbaCashout.all.order("created_at DESC")
  end

  def validate_cashout
    cashout = UbaCashout.find_by_transaction_id(params[:transaction_id])
